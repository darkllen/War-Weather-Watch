...
def get_isw_news_for_date(from_date: date, till_date: date):
    MONTH_MAP = {
        1: "january",
        2: "february",
        3: "march",
    ...
    res_list = []
    for date, news in date_news.items():
        res_list.append({"date":date, "news":convert_doc_to_vector([news])})
...
if __name__ == '__main__':
    ...
    date_news = get_isw_news_for_date(from_date, till_date)

    date_news.to_csv(f'data/{from_date} - {till_date}-isw.csv', index_label=False)